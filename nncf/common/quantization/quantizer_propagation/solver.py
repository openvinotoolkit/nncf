        ...
        for op_node_key in dom_op_node_keys:
            ...
            if op_node["op_meta"].target_input_ports is not None:
                ...
                if not all_branches_are_quantized:
                    return TransitionStatus.SHOULD_NOT_TRANSITION
            # affected_fq_amount = len(op_node[QuantizerPropagationStateGraph.AFFECTING_PROPAGATING_QUANTIZERS_ATTR])
            # dominated_edges_amount = dom_op_node_keys.count(op_node_key)
            # if affected_fq_amount < dominated_edges_amount:
            #    # Number of quantizers is less than number of
            #    # dominated edges which mean that some
            #    # branches should not be quantized. Thus
            #    # the quantizers merge is not possible
            #    return TransitionStatus.SHOULD_NOT_TRANSITION
