...
class KDLossCalculatorOld(PTCompressionLoss):
...
@COMPRESSION_ALGORITHMS.register('knowledge_distillation_old')
class KnowledgeDistillationBuilderOld(PTCompressionAlgorithmBuilder):
    ...
    def build_controller(self, target_model):
        return KnowledgeDistillationControllerOld(target_model, self.original_model, self.scale, self.is_softmax)


class KnowledgeDistillationControllerOld(PTCompressionAlgorithmController):
    ...
    def __init__(self, target_model, original_model, scale, is_softmax):
        ...
        self._loss = KDLossCalculatorOld(original_model, scale, is_softmax)
