...
class ONNXBiasCorrectionAlgorithm(BiasCorrectionAlgorithm):
    def apply(self, compressed_model: CompressedModel, engine: ONNXEngine):
        ...
        original_model = compressed_model.original_model
        quantized_model = compressed_model.compressed_model
        layers_with_biases = self._get_layers_with_biases(compressed_model)

        layers_bias_shifts = {}
        for layer in layers_with_biases:
            bias_shift = self._get_output_of_layer(compressed_model, layer, original_model, quantized_model)
            layers_bias_shifts[layer] = bias_shift
            command = ONNXUpdateBias(layer.layer_name, bias_shift)
            transformation_layout.register(command)

            model = self.model_transformer._apply_transformation(command)
            onnx.save(self.compressed_model.compressed_model, '/home/aleksei/tmp/onnx/onnx_ptq_api/test.onnx')

        return model

    def get_layers_for_statistics(self, compressed_model: CompressedModel) -> List[LayerStatistic]:
        return self._get_outputs_with_biases(compressed_model)

    def get_transformation_commands(self, a):
        pass

    def _get_outputs_with_biases(self, compressed_model: CompressedModel):
        output = []
        original_model_onnx_graph = ONNXGraph(compressed_model.original_model)
    ...
    def _get_layers_with_biases(self, compressed_model):
        output = []
        original_model_onnx_graph = ONNXGraph(compressed_model.original_model)
    ...
    def _get_output_of_layer(self, compressed_model, layer_name, model, quantized_model):
