...
from nncf.experimental.torch.sparsity.movement.layers import MovementSparsifier, SparseStructure
...
class StructuredMaskHandler:
    ...
    def _create_structured_mask_context_groups(
        ...
        for group_id, group in enumerate(pruning_groups):
            ...
            prefix_warning = 'Automatically found structured pruning group does not match the given unstructured '\
                ...
                if module in module_vs_sparse_module_info_map:
                    ...
                    if sparsifier.sparse_structure == SparseStructure.PER_DIM:
                        ...
                        if sparse_axis != producer_info.pruning_dimension:
                            nncf_logger.warning(f'{prefix_warning}. Unstructured pruning is defined for '
                    ...
                    ctx = StructuredMaskContext(sparsifier,
