...
class ONNXBiasCorrectionAlgoBackend(BiasCorrectionAlgoBackend):
    ...
    def channel_axis_by_types(self) -> Dict[ONNXOpMetatype, int]:
        return {
            ...
            ONNX_OPERATION_METATYPES.get_operator_metatype_by_op_name('MatMul'): -1,
    ...
    def is_quantized_weights(biased_node: NNCFNode, nncf_graph: NNCFGraph) -> bool:
        input_edges = nncf_graph.get_input_edges(biased_node)
        weight_port_id = biased_node.metatype.weight_definitions.weight_port_id
        weight_node = input_edges[weight_port_id].from_node
        return weight_node.metatype == ONNX_OPERATION_METATYPES.get_operator_metatype_by_op_name('DequantizeLinear')
