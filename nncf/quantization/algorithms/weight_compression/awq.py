...
class AWQ(Algorithm):
    ...
    def apply(
        ...
        for k, awq_data_item in track(awq_data.items(), description="Applying AWQ"):
            ...
            if merge_node.node_type is 'MatMul':
    ...
    def _apply_nonlinear_pattern(
        ...
        inference_nncf_graph = transform_to_inference_graph(deepcopy(graph), [], [], [])
        nx_graph = inference_nncf_graph.get_nx_graph_copy()
        for _, pattern_graph in self._patterns_ext.items():
            matches.extend(find_subgraphs_matching_pattern(nx_graph, pattern_graph(), strict=False))
        ...
        for match in matches:
            target_node = graph.get_node_by_key(match[-1])
            if not self._backend_entity.is_node_with_weights(target_node, graph):
                continue
            target_node_names = []
            for weight_op_friendly_name, _ in self._backend_entity.get_weight_names_and_port_ids(target_node, graph):
                target_node_names.append(weight_op_friendly_name)
