...
from nncf.openvino.pot.qunatization.accuracy_aware import NMSEBasedAccuracyAware
...
def _create_statistics_collector_config(
    statistics_collector_params: StatisticsCollectorParameters) -> Dict[str, Any]:
    """
    ...
    if statistics_collector_params.statistics_type in [
            StatisticsType.QUANTILE, StatisticsType.ABS_QUANTILE]:
        config['outlier_prob'] = statistics_collector_params.quantile_outlier_prob

    return config


def _create_range_estimator_config(
    range_estimator_params: RangeEstimatorParameters) -> Dict[str, Any]:
    """
...
def _create_quantization_group_config(quantization_params: QuantizationParameters,
                                      range_estimator_params: RangeEstimatorParameters,
                                      backend_params: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def _create_quantization_config(
    ...
    advanced_parameters: Optional[AdvancedQuantizationParameters]) -> Dict[str, Any]:
    """
    ...
    backend_activations_parameters = advanced_parameters.backend_params.get(
        BackendParameters.ACTIVATIONS, {})
    ...
    backend_weights_parameters = advanced_parameters.backend_params.get(
        BackendParameters.WEIGHTS, {})
    weights_config = _create_quantization_group_config(
        advanced_parameters.weights_quantization_params, advanced_parameters.
        weights_range_estimator_params, backend_weights_parameters)
    if weights_config:
...
def _create_engine_config(
    ...
    advanced_parameters: AdvancedQuantizationParameters) -> Dict[str, Any]:
    """
...
def quantize_impl(model: ov.Model,
    ...
    algorithm_parameters = _create_quantization_config(
        ...
        ignored_scope, advanced_parameters)
...
def _create_accuracy_restorer_config(
    max_drop: float,
    advanced_parameters: Optional[AdvancedAccuracyRestorerParameters]) -> Dict[str, Any]:
    """
...
def quantize_with_accuracy_control_impl(
    ...
    algotrithm_parameters = _create_accuracy_restorer_config(
        max_drop, advanced_accuracy_restorer_parameters)

    algotrithm_parameters.update(
        _create_quantization_config(
            target_device, preset, subset_size, fast_bias_correction, model_type,
            ignored_scope, advanced_quantization_parameters))

    algorithms = [
        {
            'name': 'NMSEBasedAccuracyAware',
            'params': algotrithm_parameters
        }
    ]
