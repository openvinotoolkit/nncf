...
from nncf.quantization.layers import QuantizationMode, QuantizerConfig, SymmetricQuantizer, BaseQuantizer, \
    BlockfpQuantizer, QUANTIZATION_MODULES
...
from nncf.dynamic_graph.operator_metatypes import FoldedConv2dSubtype
...
def compare_qconfigs(config: QuantizerConfig, quantizer: BaseQuantizer):
    ...
    if config.mode == QuantizationMode.BLOCKFP:
        assert config.exponent_bits == quantizer.exponent_bits
        assert config.bits == quantizer.bits
        assert config.block_size == quantizer.block_size
        assert config.is_weights or config.folded == quantizer.folded_config # Not strictly true if is_weights


def test_quantizer_config_bfp():
    bfp_config = QuantizerConfig.create(mode=QuantizationMode.BLOCKFP,
                                        exponent_bits=5,
                                        bits=5,
                                        block_size=32)
    config = QuantizerConfig.create()
    asymmetric_config = QuantizerConfig.create(mode=QuantizationMode.ASYMMETRIC)

    assert bfp_config.mode == QuantizationMode.BLOCKFP
    assert bfp_config.exponent_bits == 5
    assert bfp_config.bits == 5
    assert bfp_config.block_size == 32

    assert config.mode == QuantizationMode.SYMMETRIC
    assert config.bits == 8

    assert asymmetric_config.mode == QuantizationMode.ASYMMETRIC
    assert asymmetric_config.bits == 8
...
def test_quantizer_config_str_bfp():
    ...
    assert str(bfp_config) == "Bits:5 Exp:5 Blk:32 M:BFP SGN:S W:N PC:Y"
...
def test_quantizer_config_lt_bfp():
    bfp_config = QuantizerConfig.create(mode=QuantizationMode.BLOCKFP,
                                        ...
                                        bits=5,
                                        block_size=32)
    bfp_config_other = QuantizerConfig.create(mode=QuantizationMode.BLOCKFP,
                                              exponent_bits=7,
                                              bits=5,
                                              block_size=32)
    config = QuantizerConfig.create()
...
def test_quantization_configs_bfp():
    ...
    ref_weight_qconfig = QuantizerConfig.create(mode=QuantizationMode.BLOCKFP,
                                                exponent_bits=5,
                                                bits=5,
                                                block_size=32,
                                                is_weights=True)
    ...
    ref_activation_qconfig = QuantizerConfig.create(mode=QuantizationMode.BLOCKFP,
                                                    exponent_bits=5,
                                                    bits=5,
                                                    block_size=32,
                                                    is_weights=False)
...
def test_quantization_configs__bfp_folding():
    config = QuantizerConfig.create(mode=QuantizationMode.BLOCKFP,
                                    exponent_bits=5,
                                    bits=5,
                                    block_size=32,
                                    is_weights=False,
                                    folded=True)
    config_weights = QuantizerConfig.create(mode=QuantizationMode.BLOCKFP,
                                            exponent_bits=5,
                                            bits=5,
                                            block_size=32,
                                            is_weights=True,
...
def test_quantization_configs__with_defaults():
    ...
    ref_weight_qconfig = QuantizerConfig.create(bits=8,
    ...
    ref_activation_qconfig = QuantizerConfig.create(bits=8,
                                                    mode=QuantizationMode.SYMMETRIC,
                                                    signedness_to_force=None,
                                                    per_channel=False,
                                                    input_shape=None,
...
def test_quantization_configs__custom():
    ...
    ref_weight_qconfig = QuantizerConfig.create(bits=4,
    ...
    ref_activation_qconfig = QuantizerConfig.create(bits=4,
                                                    mode=QuantizationMode.ASYMMETRIC,
