...
class TestCollectedStatistics:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                MinMaxStatisticCollector,
                {
                    (0, 1): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[-4.0]])), max_values=Tensor(tf.constant([[6.1]]))
                    ),
                    (1,): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[1.0], [-4.0], [4.0]])),
                        max_values=Tensor(tf.constant([[4.5], [4.0], [6.1]])),
                    ),
                    (0,): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[-1.3, -4.0, -3.5]])),
                        max_values=Tensor(tf.constant([[4.5, 5.8, 6.1]])),
                    ),
            ...
            (
                ...
                {
                    (0, 1): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[-3.5]])), max_values=Tensor(tf.constant([[6.05]]))
                    ),
                    (1,): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[1.8], [-3.5], [4.15]])),
                        max_values=Tensor(tf.constant([[3.75], [3.5], [6.05]])),
                    ),
                    (0,): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[-1.15, -3, -3.25]])),
                        max_values=Tensor(tf.constant([[4.25, 5.4, 6.05]])),
                    ),
                },
            ),
            (
                ...
                {
                    (0, 1): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[-4.0]])), max_values=Tensor(tf.constant([[6.05]]))
                    ),
                    (1,): MinMaxTensorStatistic(
                        min_values=Tensor(tf.constant([[1.0], [-4.0], [4.0]])),
                        max_values=Tensor(tf.constant([[3.75], [3.5], [6.05]])),
    ...
    def test_collected_statistics_with_shape_convert(
        ...
        for reduction_axes in reduction_axes_vs_ref_statistic.keys():
            ...
            for input_ in TestCollectedStatistics.REF_INPUTS:
                collector_obj.register_input(Tensor(input_))
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                MedianMADStatisticCollector,
                {
                    (0, 1): MedianMADTensorStatistic(
                        median_values=Tensor(tf.constant([[2.8]])), mad_values=Tensor(tf.constant([[2.6]]))
                    ),
                    (1,): MedianMADTensorStatistic(
                        median_values=Tensor(tf.constant([[2.8], [-2.5], [5.4]])),
                        mad_values=Tensor(tf.constant([[0.85], [1.1], [0.65]])),
                    ),
                    (0,): MedianMADTensorStatistic(
                        median_values=Tensor(tf.constant([[2.5, 2.3, 3.35]])),
                        mad_values=Tensor(tf.constant([[1.9, 3.1, 2.7]])),
                    ),
                    # Not supported for now:
            ...
            (
                ...
                {
                    (0, 1): PercentileTensorStatistic({10.0: Tensor(tf.constant([[-3.15]]))}),
                    (1,): PercentileTensorStatistic({10.0: Tensor(tf.constant([[1.5], [-3.75], [4.15]]))}),
                    (0,): PercentileTensorStatistic({10.0: Tensor(tf.constant([[-1.15, -3, -3.25]]))}),
                    # Not supported for now:
            ...
            (
                ...
                {
                    (0, 1): PercentileTensorStatistic({10.0: Tensor(tf.constant([[-2.9]]))}),
                    (1,): PercentileTensorStatistic({10.0: Tensor(tf.constant([[2.0100], [-3.3500], [4.4000]]))}),
                    (0,): PercentileTensorStatistic({10.0: Tensor(tf.constant([[-0.3900, -1.9400, -1.9300]]))}),
                    # Not supported for now:
                    # (3, 3): PercentileTensorStatistic(
                    #     {
                    #         10.0: tf.constant([
    ...
    def test_collected_samples(self, collector_for_interface_test: TensorStatisticCollectorBase):
        for input_ in TestCollectedStatistics.REF_INPUTS:
            collector_for_interface_test.register_input(Tensor(input_))
    ...
    def test_num_samples(self, collector_for_num_samples_test: OfflineTensorStatisticCollector):
        for input_ in TestCollectedStatistics.REF_INPUTS * 10:
            collector_for_num_samples_test.register_input(Tensor(input_))
