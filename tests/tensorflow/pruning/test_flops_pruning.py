...
@pytest.mark.parametrize(
    ("model_fn", "all_weights", "pruning_flops_target", "ref_full_flops", "ref_current_flops",
     "ref_full_params", "ref_current_params", "ref_sizes", "ref_num_of_sparse"),
    (
        (get_test_model_shared_convs, True, 0.4, 461438976, 276858560, 11534848, 6920562,
            [{'conv1': 410}, {'conv2': 3382272}], {'conv1': 102, 'conv2^1': 290, 'conv2^0': 290}),
        (get_test_model_shared_convs, False, 0.4, 461438976, 275300352, 11534848, 6881664,
            [{'conv1': 384}, {'conv2': 3538944}], {'conv1': 128, 'conv2^1': 256, 'conv2^0': 256}),
        ...
        (get_broadcasted_linear_model, True, 0.9, 167936, 15872, 33568, 1384,
        ...
        (get_diff_cluster_channels_model, False, 0.0, 8523776, 8523776, 4227616, 4227616,
            [{'first_conv': 32}], {'first_conv': 0}),
    )
)
def test_flops_calulation_for_spec_layers(model_fn, all_weights, pruning_flops_target,
                                          ref_full_flops, ref_current_flops,
                                          ref_full_params, ref_current_params, ref_sizes,
                                          ref_num_of_sparse):
