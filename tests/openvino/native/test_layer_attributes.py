...
def get_group_conv(node_name, input_shape):
    shape = (input_shape[1] // 2, input_shape[1], 2, 1, 1)
    kernel = opset.constant(np.ones(shape), dtype=np.float32, name="Const")
    return get_depthwise_conv(node_name, input_shape, kernel)


def get_depthwise_conv(node_name, input_shape, kernel=None):
    strides = [1, 2]
    pads = [0, 1]
    dilations = [3, 1]
    if kernel is None:
...
def get_matmul_b_swapped(node_name, input_shape):
    return get_matmul(node_name, input_shape, transpose_b=True, swap_inputs=True)


def get_matmul_a_swapped(node_name, input_shape):
    return get_matmul(node_name, input_shape, transpose_a=True, swap_inputs=True)


def get_matmul(node_name, input_shape, transpose_a=False, transpose_b=False, swap_inputs=False):
    ...
    input_ = opset.parameter(input_shape, name="Input0")
    a, b = (data, input_) if swap_inputs else (input_, data)
    output = opset.matmul(a, b, transpose_a=transpose_a, transpose_b=transpose_b, name=node_name)
    return [input_], [output]
...
def get_1d_matmul(node_name, input_shape):
    data_shape = (input_shape[-1],)
    data = opset.constant(np.ones(tuple(data_shape)), dtype=np.float32, name="Const")
    input_ = opset.parameter(input_shape, name="Input0")
    output = opset.matmul(input_, data, transpose_a=False, transpose_b=False, name=node_name)
    return [input_], [output]


def get_add(node_name, input_shape):
    data_shape = [1] * len(input_shape)
    data = opset.constant(np.ones(tuple(data_shape)), dtype=np.float32, name="Const")
    input_ = opset.parameter(input_shape, name="Input0")
    output = opset.add(input_, data, name=node_name)
    return [input_], [output]


def get_lstm(node_name, input_shape):
