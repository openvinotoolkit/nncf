...
class LMLinearModel(OVReferenceModel):
    OUTPUT_DIM = 32
    INPUT_SHAPE = [24, 16]  # [SeqLen, HiddenDim]
    ...
    def _create_ov_model(self, transpose_b: bool = True, transpose_a=False, input_shape=None):
        ...
        hdim_axis = 0 if transpose_a else -1
...
def test_compression_with_3D_activations(kwargs):
    dataset_size = 4
    model = LMLinearModel(input_shape=[3, 5, 16]).ov_model
...
def test_compression_with_transposed_activations(kwargs):
    dataset_size = 4
    model = LMLinearModel(input_shape=[24, 16], transpose_a=True, transpose_b=False).ov_model
    input_data = [np.ones(inp.shape) for inp in model.inputs] * dataset_size
    dataset = Dataset(input_data)
