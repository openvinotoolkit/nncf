...
TRANSFORMERS_COMMIT = "fbc5bf10cfe4d4ca81f8daacc148b0abd51dda5a"
MMDETECTION_COMMIT = "c77ccbbf235c0eb50a4440698eefc2ae199f837f"
...
class TestTransformers:
    ...
    def test_xnli_train(self, temp_folder):
        com_line = "examples/run_xnli.py --model_type bert --model_name_or_path bert-base-chinese --language" \
                   " zh --train_language zh --do_train --data_dir {} --per_gpu_train_batch_size 24" \
                   " --learning_rate 5e-5 --num_train_epochs 1.0 --max_seq_length 128 --output_dir {}" \
                   " --save_steps 200 --nncf-config nncf_bert_config_xnli.json"\
    ...
    def test_xnli_eval(self, temp_folder):
        com_line = "examples/run_xnli.py --model_type bert --model_name_or_path {output} --language zh" \
                   " --do_eval --data_dir {} --learning_rate 5e-5 --max_seq_length 128 --output_dir {output}" \
                   " --nncf-config nncf_bert_config_xnli.json --per_gpu_eval_batch_size 24"\
    ...
    def test_squad_train(self, temp_folder):
        com_line = "examples/run_squad.py --model_type bert --model_name_or_path " \
                   ...
                   "{} --per_gpu_train_batch_size=1 --save_steps=200 --nncf-config" \
    ...
    def test_squad_eval(self, temp_folder):
        com_line = "examples/run_squad.py --model_type bert --model_name_or_path {output} --do_eval" \
                   " --do_lower_case  --predict_file {}/squad/dev-v1.1.json --learning_rate 3e-5" \
                   " --max_seq_length 384 --doc_stride 128 --per_gpu_eval_batch_size=4 --output_dir {output} " \
                   "--nncf-config nncf_bert_config_squad.json"\
    ...
    def test_glue_train(self, temp_folder):
        com_line = "examples/run_glue.py --model_type roberta --model_name_or_path roberta-large-mnli" \
                   " --task_name mnli --do_train --data_dir {}/glue/glue_data/MNLI" \
                   " --per_gpu_train_batch_size 4 --learning_rate 2e-5 --num_train_epochs 1.0 --max_seq_length 128 " \
                   "--output_dir {} --save_steps 200 --nncf-config" \
    ...
    def test_glue_eval(self, temp_folder):
        com_line = "examples/run_glue.py --model_type roberta --model_name_or_path {output}" \
                   ...
                   " --nncf-config nncf_roberta_config_mnli.json"\
    ...
    def test_glue_distilbert_train(self, temp_folder):
        com_line = "examples/run_glue.py --model_type distilbert --model_name_or_path distilbert-base-uncased" \
                   " --task_name SST-2 --do_train --do_lower_case --max_seq_length 128 --per_gpu_train_batch_size 8" \
    ...
    def test_glue_distilbert_eval(self, temp_folder):
        com_line = "examples/run_glue.py --model_type distilbert --model_name_or_path {output}" \
                   " --task_name SST-2 --do_eval --do_lower_case --max_seq_length 128" \
                   " --output_dir {output} --data_dir {}/glue/glue_data/SST-2" \
                   " --nncf-config nncf_distilbert_config_sst2.json"\
    ...
    def test_convert_to_onnx(self, temp_folder):
        com_line = "examples/run_squad.py --model_type bert --model_name_or_path {output} --output_dir {output}" \
                   " --to-onnx {output}/model.onnx".format(output=os.path.join(temp_folder["models"], "squad"))
        runner = Command(create_command_line(com_line, self.activate_venv, self.trans_python,
                                             self.cuda_visible_string), self.TRANS_PATH)
        res = runner.run()
        assert res == 0
...
class TestMmdetection:
    ...
    def test_install_mmdet(self):
        ...
        subprocess.run("{}; git checkout {}".format(self.activate_venv, MMDETECTION_COMMIT), check=True, shell=True,
                       cwd=self.MMDET_PATH)
        subprocess.run("{}; pip install mmcv".format(self.activate_venv), check=True, shell=True, cwd=self.MMDET_PATH)
        subprocess.run("{}; pip install -r requirements/build.txt".format(self.activate_venv), check=True, shell=True,
                       cwd=self.MMDET_PATH)
        subprocess.run("{}; pip install \"git+https://github.com/cocodataset/cocoapi.git#subdirectory=PythonAPI\""
                       .format(self.activate_venv),
    ...
    def test_retinanet_eval(self):
        ...
        comm_line = "tools/test.py configs/retinanet/retinanet_r50_fpn_1x_int8.py {} --eval segm".format(checkpoint)
        runner = Command(create_command_line(comm_line, self.activate_venv, self.mmdet_python), self.MMDET_PATH)
        res = runner.run()
        assert res == 0
