...
class TemplateTestReducersAggreagtors:
    ...
    REF_MAD_PERCENTILE_REF_VALUES_DYNAMIC_TENSORS = {
        MedianAbsoluteDeviationAggregator: {
            "median_values": np.array([[[28.5, 35.5, 43.5]]]).reshape(1, 1, 3, 1),
            "mad_values": np.array([[[[24.0, 24.0, 24.0]]]]).reshape(1, 1, 3, 1),
        },
        PercentileAggregator: {
            5: np.array([[[[0.95, 5.95, 9.95]]]]).reshape(1, 1, 3, 1),
            10: np.array([[[[1.9, 7.9, 15.5]]]]).reshape(1, 1, 3, 1),
            90: np.array([[[[75.1, 83.1, 91.1]]]]).reshape(1, 1, 3, 1),
            95: np.array([[[[77.05, 85.05, 93.05]]]]).reshape(1, 1, 3, 1),
        },
    }
