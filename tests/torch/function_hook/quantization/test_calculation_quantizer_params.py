...
def calculate_fq_params(model, input_data):
    ...
    return {
        "/x__0.0/FakeQuantize": conv1_stats,
        "/bn1/bn1/batch_norm/0__0.0/FakeQuantize": bn1_stats,
        "/avg_pool/avg_pool/adaptive_avg_pool2d/0__0.0/FakeQuantize": (avg_pool_stats),
        "/conv2/conv2/conv2d/0__0.0/FakeQuantize": conv2_stats,
        "/conv1/conv1/conv2d/0__1.0/FakeQuantize": conv1_w_stats,
        "/conv2/conv2/conv2d/0__1.0/FakeQuantize": conv2_w_stats,
    }
...
def test_quantizer_parameters_export(tmp_path: Path, _seed):
    ...
    statistics_aggregator = PT2StatisticsAggregator(dataset)

    wrapped_model: GraphModelWrapper = wrap_model(model, input_data, True)
    statistic_points = min_max_algo.get_statistic_points(wrapped_model.model, wrapped_model.get_graph())
    statistics_aggregator.register_statistic_points(statistic_points)
    statistics_aggregator.collect_statistics(wrapped_model, wrapped_model.get_graph())
    torch_quantized_model = min_max_algo.apply(
        wrapped_model, wrapped_model.get_graph(), statistics_aggregator.statistic_points
    )
