...
def test_graph_pattern_io_building():
    ...
    mock_graph.add_edges_from([('A', 'B'), ('A', 'D'), ('B', 'C'), ('C', 'D'), ('D', 'E'),
                               ('D', 'F'), ('D', 'G'), ('E', 'H')])
    mark_input_ports_lexicographically_based_on_input_node_key(mock_graph)

    graph = get_nncf_graph_from_mock_nx_graph(mock_graph)

    def make_mock_edge(from_node_name: NNCFNodeName, to_node_name: NNCFNodeName,
                       input_port_id: int, output_port_id: int):
        return NNCFGraphEdge(get_node(from_node_name),
    ...
    ref_patterns_and_ios = [
        (['/A_0', '/B_0'], NNCFGraphPatternIO(input_edges=[],
                                              output_edges=[
                                                  make_mock_edge('/B_0', '/C_0', input_port_id=0, output_port_id=0),
                                                  make_mock_edge('/A_0', '/D_0', input_port_id=0, output_port_id=1)])),
        (['/C_0'], NNCFGraphPatternIO(input_edges=[make_mock_edge('/B_0', '/C_0', input_port_id=0, output_port_id=0)],
                                      output_edges=[
                                          make_mock_edge('/C_0', '/D_0', input_port_id=0, output_port_id=0)])),
        (['/A_0', '/B_0', '/C_0'], NNCFGraphPatternIO(input_edges=[],
                                                      output_edges=[make_mock_edge('/C_0', '/D_0', input_port_id=0,
                                                                                   output_port_id=0),
                                                                    make_mock_edge('/A_0', '/D_0', input_port_id=0,
                                                                                   output_port_id=1)])),
        (['/D_0'], NNCFGraphPatternIO(input_edges=[make_mock_edge('/C_0', '/D_0', input_port_id=0, output_port_id=0),
                                                   make_mock_edge('/A_0', '/D_0', input_port_id=0, output_port_id=1)],
                                      output_edges=[make_mock_edge('/D_0', '/E_0', input_port_id=0, output_port_id=0),
                                                    make_mock_edge('/D_0', '/F_0', input_port_id=0, output_port_id=1),
                                                    make_mock_edge('/D_0', '/G_0', input_port_id=0,
                                                                   output_port_id=2)])),
        (['/E_0', '/F_0', '/H_0'],
         NNCFGraphPatternIO(input_edges=[make_mock_edge('/D_0', '/E_0', input_port_id=0, output_port_id=0),
                                         make_mock_edge('/D_0', '/F_0', input_port_id=0, output_port_id=1)],
                            output_edges=[])),
        (['/G_0'], NNCFGraphPatternIO(input_edges=[make_mock_edge('/D_0', '/G_0', input_port_id=0, output_port_id=2)],
                                      output_edges=[]))
    ]
