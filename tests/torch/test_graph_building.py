...
class ModelForTestWithReshapeAndFlatten(ModelForTest):
    def forward(self, x):
        y = super().forward(x)
        size = y.size()
        y = y.view(size + (1, 1))
...
def test_reshape_attributes_saved_during_graph_building(input_shape):
    model = ModelForTestWithReshapeAndFlatten()
    input_info = ModelInputInfo(input_shape)
    graph_builder = GraphBuilder(create_dummy_forward_fn([input_info, ], with_input_tracing=True,
                                                         with_output_tracing=True))
    graph = graph_builder.build_graph(model)
    reshape_nodes_with_attributes = {
        '8 ModelForTestWithReshapeAndFlatten/view_0':
        ...
        '9 ModelForTestWithReshapeAndFlatten/flatten_0':
            {'input_shape': (input_shape[0], ModelForTest.OUT_CHANNELS, input_shape[2], input_shape[3], 1, 1),
             'output_shape': (input_shape[0] * ModelForTest.OUT_CHANNELS * input_shape[2] * input_shape[3],)}
    }
    reshape_nodes_without_attributes = ['10 ModelForTestWithReshapeAndFlatten/view_1']
    for key, shapes in reshape_nodes_with_attributes.items():
        node = graph.get_node_by_key(key)
        assert isinstance(node.layer_attributes, ReshapeLayerAttributes)
        assert node.layer_attributes.input_shape == shapes['input_shape']
        assert node.layer_attributes.output_shape == shapes['output_shape']

    for key in reshape_nodes_without_attributes:
        node = graph.get_node_by_key(key)
