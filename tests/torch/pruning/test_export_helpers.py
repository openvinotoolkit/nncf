...
REF_OUTPUT_MASK = [
    [torch.ones(10), 'error'],
    [torch.tensor([0, 1] * 32), torch.tensor([[0] * 32 + [1] * 32] * 32).view(-1)],
    [torch.tensor([0, 1] * 32), torch.tensor([[0] * 32 + [1] * 32] * 32).view(-1)],
    [torch.tensor([0, 1] * 32)] * 2,
]
...
def test_reshape_metatype_mask_prop(node_type, input_shape, output_shape, output_mask, output_mask_ref):
    ...
    for output_mask_cur, output_mask_ref_cur in ([(None, None), (output_mask, output_mask_ref)]):
        ...
        if isinstance(output_mask_ref_cur, str):
            ...
            PTReshape.mask_propagation(reshape_node, graph)
            assert torch.all(torch.tensor(reshape_node.data['output_mask'] == output_mask_ref_cur))
...
def test_reshape_is_last_op(node_type):
    ...
    graph = PTNNCFGraph()
    prev_node = graph.add_nncf_node('prev_node', 'linear', LinearMetatype)
    reshape_node = graph.add_nncf_node(node_name, node_type, ReshapeMetatype,
                                       layer_attributes=layer_attributes)

    assert not PTReshape.accept_pruned_input(reshape_node)
    ...
    for output_mask in (None, torch.ones(10)):
        prev_node = graph.get_node_by_id(prev_node.node_id)
        reshape_node = graph.get_node_by_id(reshape_node.node_id)
