...
TEST_MODELS_DESC = [
    ...
    pytest.param(ModelDesc("lstm_cell", LSTMCellNNCF,
                           [2, 1]), marks=SKIP_MARK),
    pytest.param(ModelDesc("lstm_uni_seq", partial(NNCF_RNN, num_layers=1, bidirectional=False),
                           [3, 1, 1]), marks=SKIP_MARK),
    pytest.param(ModelDesc("lstm_uni_stacked", partial(NNCF_RNN, num_layers=2, bidirectional=False),
                           [3, 1, 1]),  marks=SKIP_MARK),
    pytest.param(ModelDesc("lstm_bi_seq", partial(NNCF_RNN, num_layers=1, bidirectional=True),
                           [3, 1, 1]), marks=SKIP_MARK),
    pytest.param(ModelDesc("lstm_bi_stacked", partial(NNCF_RNN, num_layers=2, bidirectional=True),
                           [3, 1, 1]), marks=SKIP_MARK),
]


@pytest.mark.parametrize(
    "desc", TEST_MODELS_DESC, ids=[get_model_name(m) for m in TEST_MODELS_DESC]
)
