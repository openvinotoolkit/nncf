...
def min_max_quantize_model(
    ...
    original_model.eval()
    wrapped_model = wrap_model(original_model, torch.ones([1, 1, 10, 10]), trace_parameters=True)
    quantized_model = post_training_quantization.apply(wrapped_model, wrapped_model.get_graph(), dataset=dataset)
...
def get_fq_nodes(model: torch.nn.Module) -> dict[Scope, torch.nn.Module]:
    ...
    for name, module in model.named_modules():
        if isinstance(module, quantization_types):
            ret[name] = module
    return ret
...
def test_overflow_fix_scales(_seed, overflow_fix):
    ...
    fq_nodes = get_fq_nodes(quantized_model.model)
