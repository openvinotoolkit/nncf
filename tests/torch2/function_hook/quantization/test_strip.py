...
def test_nncf_strip_api(strip_type, do_copy):
    ...
    if strip_type == "nncf":
        strip_model = nncf.strip(quantized_model, do_copy, StripFormat.NATIVE, example_input)
    elif strip_type == "torch":
        strip_model = nncf.torch.strip(quantized_model, do_copy, StripFormat.NATIVE, example_input)
...
def test_strip_quantization(overflow_fix, tmp_path):
    model = BasicConvTestModel()
