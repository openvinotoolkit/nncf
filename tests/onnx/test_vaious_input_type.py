...
class TestMultipleInputs:
    ...
    def test_input_shape_input_keys(self, model_to_test, input_keys):
        ...
        result_input_shape, result_input_keys = infer_input_shape(model, model_to_test.input_shape, input_keys)

        assert isinstance(result_input_shape, (list, tuple)) and isinstance(result_input_keys, list)
        assert result_input_shape == model_to_test.input_shape
        assert result_input_keys[0] == input_keys

    @pytest.mark.parametrize(('model_to_test', 'input_keys'), TEST_CASES)
    def test_input_shape(self, model_to_test, input_keys):
        model = load_model(model_to_test)

        result_input_shape, result_input_keys = infer_input_shape(model, model_to_test.input_shape, None)
    ...
    def test_input_keys(self, model_to_test, input_keys):
        ...
        result_input_shape, result_input_keys = infer_input_shape(model, None, input_keys)
...
class TestDynamicInputShape:
    ...
    @pytest.mark.parametrize(('model_to_test', 'input_keys'), DET_TEST_CASES)
    def test_input_shape_input_keys(self, model_to_test, input_keys):
        model = load_model(model_to_test)
    ...
    @pytest.mark.xfail(reason="For models with dynamic input_shape, input_shape and input_keys must be set.")
    @pytest.mark.parametrize(('model_to_test', 'input_keys'), DET_TEST_CASES)
    def test_input_shape(self, model_to_test, input_keys):
        model = load_model(model_to_test)
    ...
    def test_input_keys(self, model_to_test, input_keys):
        ...
        _, _ = infer_input_shape(model, None, input_keys)
