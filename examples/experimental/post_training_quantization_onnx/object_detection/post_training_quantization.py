...
from examples.experimental.post_training_quantization_onnx.object_detection.dataloader import create_train_dataloader
...
def run(onnx_model_path: Union[str, bytes, os.PathLike], output_model_path: Union[str, bytes, os.PathLike],
        ...
        input_shape: List[int], stride: int, per_channel: bool, ignored_scopes: List[str] = None):
    ...
    train_loader = create_train_dataloader(data_loader_path, input_shape, stride)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_shape", help="Model's input shape", nargs="+", type=int, default=[1, 3, 640, 640])
    parser.add_argument("--stride", help="Yolov5 stride param", type=int, default=32)
