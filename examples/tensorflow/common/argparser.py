...
def get_common_argument_parser(**flags):
    ...
    add_argument(
        ...
        parameters=argument_parameters("--metrics-dump", type=str, help="Name of metrics collecting .json file"),
    )
    ...
    if flags.get("resume_args", True):
        ...
        add_argument(
            ...
            parameters=argument_parameters(
                "--weights", default=None, type=str, help="Path to pretrained weights in H5 format."
            ),
        )
    ...
    add_argument(
        ...
        parameters=argument_parameters(
            "--precision", type=str, default="float32", help="Precision to use {bfloat16, float32}. Default: float32"
        ),
    )
    ...
    add_argument(
        ...
        parameters=argument_parameters(
            "--dataset-type", choices=["tfds", "tfrecords"], default="tfds", help="Dataset type."
        ),
    )
    ...
    add_argument(
        ...
        parameters=argument_parameters(
            "--save-checkpoint-freq", default=5, type=int, help="Checkpoint save frequency (epochs). Default: 5"
        ),
    )
    ...
    parser.add_argument("--disable-compression", help="Disable compression", action="store_true")
    ...
    parser.add_argument(
        "--disable_tensor_float_32_execution", help="Disable execution in TensorFloat-32", action="store_true"
    )
