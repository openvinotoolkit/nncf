...
def measure_similarity(get_ov_model_for_eval_fn: Callable, tokenizer: Any, wwb_ref_file: Path) -> float:
    """
    Measures the similarity of a model's output to a reference outputs from a given file using WWB evaluation.

    :param get_ov_model_for_eval_fn: A function that creates OpenVINO model to be evaluated.
    ...
    ov_model = get_ov_model_for_eval_fn()
...
def main(argv):
    ...
    @torch.no_grad()
    def get_ov_model_for_eval_fn():
        """
        Create an OpenVINO model from the checkpoint for evaluation on CPU via WWB.
        """
        model_to_eval = AutoModelForCausalLM.from_pretrained(
            args.pretrained_model, torch_dtype=torch.float32, device_map="cpu"
        )
