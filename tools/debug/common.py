...
from nncf.torch.layers import NNCFConv1d
...
def is_weightable(layer):
    return isinstance(layer, (nn.Conv1d, nn.Conv2d, nn.Linear)) and not isinstance(
        layer, (NNCFConv1d, NNCFConv2d, NNCFLinear)
    )
...
def register_print_hooks(path, model, data_to_compare, num_layers=-1, dump_activations=False, prefix="", idx=0):
    for name, children in model.named_children():
        ...
        idx = register_print_hooks(
            path, children, data_to_compare, num_layers, dump_activations, prefix=name_full + ".", idx=idx
        )
