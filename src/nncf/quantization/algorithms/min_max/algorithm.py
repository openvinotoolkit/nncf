...
class MinMaxQuantization(Algorithm):
    ...
    def _get_ignored_names(
        ...
        autogenerated_ignored_names |= {
            node.node_name
            for node in inference_nncf_graph.get_all_nodes()
            if node.attributes.get(NNCFNode.DISABLE_GETITEM_QUANTIZER_PROPAGATION_ATTR, False)
        }
    ...
    def _get_quantizer_setup(
        ...
        allowed_post_hook_insertion_points = [
            PostHookInsertionPoint(node.node_name)
            for node in inference_nncf_graph.get_all_nodes()
            if not node.attributes.get(NNCFNode.DISABLE_GETITEM_QUANTIZER_PROPAGATION_ATTR, False)
        ]
        ip_graph = InsertionPointGraph(
            inference_nncf_graph, allowed_post_hook_insertion_points=allowed_post_hook_insertion_points
        )
