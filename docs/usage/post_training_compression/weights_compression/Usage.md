...
All embeddings, convolutions and last linear layers are always compressed to a backup precision, which is INT8_ASYM, by default. To quantize embeddings and last linear layers to 4-bit, use `all_layers=True`.
Percent of the rest layers compressed to 4-bit can be configured by "ratio" parameter. E.g. ratio=0.9 means 90% of layers compressed to the corresponding 4-bit data type and the rest to a backup precision.
...
```python
from nncf import compress_weights, BackupPrecision, CompressWeightsMode
compressed_model = compress_weights(model, mode=CompressWeightsMode.INT4_SYM, backup_precision=BackupPrecision.FP) # model is openvino.Model object
```
