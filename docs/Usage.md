...
def configure_optimizers_fn():
    ...
    is not applicable).
    '''
def dump_checkpoint_fn(model, save_dir, accuracy_aware_metainfo):
    '''
    An (optional) function that saves the model's checkpoint. 
    The user should define how to save the checkpoint.
    `save_dir` is a directory that Accuracy-Aware pipeline is created to store logs.  
    `accuracy_aware_metainfo` is all metainfo that generated by the algorithm, that can be useful to save
     such as current epoch number, compression_state, optimizer, scheduler and etc.
    '''
```
