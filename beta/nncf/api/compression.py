...
class CompressionAlgorithmController:
    ...
    def get_complete_loss(self, model_loss):
        '''
        Returns complete loss function in fashion of
        loss(y_true, y_pred) = model_loss(y_true, y_pred)
                                            + compressed_loss()
        Arguments:
            `model_loss` - loss function of model
        '''
        class CompleteLoss(tf.keras.losses.Loss):

            def __init__(self, compression_loss):
                ...
                name = model_loss.name + '_plus_compression_loss'
                super().__init__(reduction, name)

            def call(self, y_true, y_pred):
                return model_loss(y_true, y_pred) + self.compression_loss()

        return CompleteLoss(self._loss)
